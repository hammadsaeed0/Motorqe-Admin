...
const ViewContactModal = ({ isOpen, toggle, newsData }) => {
  if (!newsData) {
    ...
    <Modal isOpen={isOpen} toggle={toggle}>
      ...
      <ModalBody>
        <div className="mb-3">
          ...
          <span className="text-black bg-light p-2 d-block">{title}</span>
        </div>
        <div>
          <strong className="text-success">Content:</strong>{" "}
          <span className="text-black bg-light p-2 d-block">{content}</span>
        </div>
